...
#define VERIFY_(result, expression)
#else
...
public:
    Backup(Connection const& destination,
        ...
        char const* const sourceName = "main") : m_handle(sqlite3_backup_init(destination.GetAbi(),
            destinationName,
            source.GetAbi(),
            sourceName)),
...
struct Reader {
    ...
    wchar_t const* GetWideString(int const column = 0) const noexcept {
        return reinterpret_cast<const char*>(sqlite3_column_text16(
            static_cast<T const*>(this)->GetAbi(), column));
    }
    int GetStringLenght(int const column = 0) const noexcept {
        return sqlite3_column_bytes(static_cast<T const*>(this)->GetAbi(), column);
    }
    int GetWideStringLenght(int const column = 0) const noexcept {
        return sqlite3_column_bytes(static_cast<T const*>(this)->GetAbi(), column) / sizeof(wchar_t);
    }
...
public:
    ...
    void Bind(int const index, wchar_t const* const value, int size = -1) const {
...
public:
    ...
    bool operator !=(RowIterator const& x) const noexcept {
        return m_statement != x.m_statement;
    }
