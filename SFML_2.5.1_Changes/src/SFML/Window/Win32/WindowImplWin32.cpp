...
// Set that this process is DPI aware and can handle DPI scaling
    ...
    else
    {
        ...
        if (style & Style::Modern) {
            additional_todo = true;
            win32Style = WS_POPUP | WS_THICKFRAME | WS_CAPTION  | WS_SYSMENU | WS_MAXIMIZEBOX | WS_MINIMIZEBOX;
        }
        if (style & Style::Resize) {
            resizable_modern = true;
        }
#endif
        /// </MEP Modification>
    }
    ...
    if (additional_todo) {
        ...
        DwmExtendFrameIntoClientArea(m_handle, &shadow_state);
        SetWindowPos(m_handle, nullptr, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_FRAMECHANGED);
...
{
    /// <MEP Modification>
    //The problem here is that we are simply hiding the borders from WS_CAPTION
    //Thats why to change a size we simple need to do that:
    if (additional_todo) {
...
/// <MEP Modification>
LRESULT WindowImplWin32::customAreaTest(POINT cursor) const {
    const POINT border{
...
LRESULT CALLBACK WindowImplWin32::globalOnEvent(HWND handle, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    if (window)
    {
        ...
        /// <MEP Modification>
        if (window->additional_todo) {
            if (message == WM_NCCALCSIZE){
                if (wParam == TRUE)
                    return 0;
            } 
        }
        if (window->resizable_modern) {
            if (message == WM_NCHITTEST) {
                // When we have no border or title bar, we need to perform our
                // own hit testing to allow resizing and moving.
                return window->customAreaTest(POINT{ GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam) });
            }
        }
        /// </MEP Modification>
