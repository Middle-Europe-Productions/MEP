...
TreeRenderer::TreeRenderer(MEP::Window::Template::Application& base) :
	...
	//4---
		base.getMepView(), //This is a view of the current window starting position.
		base.getMepView() //This is the view of the master window.
	//----
	),
	...
	//-----
{
	...
	//-----
	//4----
	//Sometimes we do want our objects to automatically rescale.
	//There are three methods: addMethodScale, addMethodPos, addMethodRect.
	//This is not that simple because we do need to tell our compiler what to do.
	//Lets provide a formula:
	object.addMethodScale(
		[&base]()->sf::Vector2f {
			return sf::Vector2f((float)base.getResolution().x/500.f,
				(float)base.getResolution().y/500.f);
		}
	);
	//Rescaling is out of the DrawTags of the objects.
	//U do need to add the DrawTag to execute the formula.
	object.addDrawTag(MEP::DrawTag::Resize_Scale);
	//Secondly, let us create a new static object.
	//We will attach is to the bottom right corner of the window.
	//Here is how you do it.
	//1. Declare the object.
	static_object = new MEP::TextureObject(base.get<MEP::Object>(1), 
		{ 
			(float)base.getResolution().x - (float)base.get<MEP::Object>(1).getSize().x,
			(float)base.getResolution().y - (float)base.get<MEP::Object>(1).getSize().y,
		}
		);
	static_object->addDrawTag(MEP::DrawTag::ViewLock);
	newObject(static_object);
	//After this part the objects will ignore our custom view, hovewer, it will not change the position if the window is resized. 
	//Let's fix that.
	static_object->addMethodPos([&base, this]()->sf::Vector2f
		{
			return {(float)base.getResolution().x - static_object->getSize().x,
					(float)base.getResolution().y - static_object->getSize().y};
		});
	static_object->addDrawTag(MEP::DrawTag::Resize_Pos);
	//Finally, we can paint the object.
	//Let's do this a bit differently. With alpha channel!. Full RGBA is avalible for u.
	static_object->setColor({ 100, 0, 200, 120 });
	//To demonstrate the idea of following animations we do need to create our follow_object;
	follow_object = new MEP::TextureObject(base.get<MEP::Object>(1));
	//if you do not want to doplicate all of the formulas u can use << operator 
	//which is going to change position, scale.
	*follow_object << object;
	//U can use the move method to change the object position relative to the current position.
	follow_object->movePosition({ 100, 0 });
	//Second animation is going to have the same attributes.
	second_animation = new MEP::AnimationPosition(*animation);
	follow_object->setFollow(*second_animation, MEP::Following::FollowType::X_Pos);
	newObjects(second_animation, follow_object);
	//-----
}
...
void TreeRenderer::handleEvent(sf::RenderWindow& Window, sf::Event& event)
{
	...
	if (event.type == sf::Event::KeyPressed) {
		...
		//B will activate the animation.
			...
			//4---
			//U do need to use command setDelay instead of run to execute the run the animation with the delay.
			second_animation->setDelay(sf::milliseconds(1000), MEP::Direction::Forward);
			//----
		}
	...
	//4---
		...
		if (event.mouseButton.button == sf::Mouse::Left) {
			//U can use the MEP::WindowBase build in variables.
